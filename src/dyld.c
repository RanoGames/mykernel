...
#define DYLD_MAIN_MAX  0x100000u
...
static enum dyld_result load_segments(const uint8_t* image, size_t size,
    ...
    for (uint16_t i = 0; i < eh->e_phnum; i++) {
        ...
        if (check_main_window) {
            if (addr < DYLD_MAIN_BASE ||
                addr + ph->p_memsz > DYLD_MAIN_BASE + DYLD_MAIN_MAX)
                return DYLD_ERR_ELF;
        }
...
static enum dyld_result load_needed(struct module* m) {
    ...
    for (struct elf32_dyn* d = m->dynamic; d->d_tag != DT_NULL; d++) {
        ...
        int found = 0;
        for (int i = 0; i < mod_count; i++)
            if (str_eq(mods[i].name, libname)) { found = 1; break; }
        if (found) continue;

        terminal_writestring("dyld: load ");
        terminal_writestring(libname);
        terminal_putchar('\n');
        ...
        size_t i = 0;
        while (libname[i] && i + 6 < sizeof(path) - 1) {
            path[5 + i] = libname[i];
            i++;
        }
        path[5 + i] = '\0';
        ...
        if (fs_read_path(path, &content, &len) != FS_OK) {
            /* basename only */
            if (fs_read_path(libname, &content, &len) != FS_OK) {
        ...
        uint32_t bias = DYLD_LIB_BASE + (uint32_t)mod_count * DYLD_LIB_STRIDE;
        enum dyld_result r = load_one(libname, (const uint8_t*)content, len, bias, 0, 0);
        if (r != DYLD_OK) return r;
    }
    return DYLD_OK;
}
