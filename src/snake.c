/* snake.c — Змейка на VGA Mode 13h
 * Управление: стрелки, WASD; Q/ESC — выход
 * Мышь: курсор-точка (декоративно)
 */
...
void snake_run(void) {
    ...
    while (running) {
        ...
        /* сброс очереди мыши (курсор всё равно обновляется в draw) */
        struct mouse_event mev;
        while (mouse_poll_event(&mev))
            ;

        uint32_t now = timer_ticks();
        /* скорость: каждые ~15 тиков (~150ms при 100Hz) */
